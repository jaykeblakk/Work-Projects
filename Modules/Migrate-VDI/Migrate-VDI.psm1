function Migrate-VDI()
{
    ...
    $machines = Import-Excel "C:\temp\vdi list.xlsx" | where datastore -notlike "*Example Datastore*" | select machine 
    $jobs = foreach ($machine in $machines)
    {
        start-threadjob {
            ...
            if ($vm.base.basicstate -like "AVAILABLE")
            {
                ...
                $destination = get-vmhost | where name -like "*Example*" | sort-object freespacegb -descending | select -first 1
                $networkAdapter = get-networkadapter $vdi
                $destinationportgroup = Get-VDPortgroup -VDSwitch "DSwitch_Example" -name "VDI"
                $destinationDatastore = get-datastore | where name -like "*Example*" | sort-object freespacegb -descending | select -first 1
